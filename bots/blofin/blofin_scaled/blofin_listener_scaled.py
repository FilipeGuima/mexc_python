...
def adjust_price_to_step(price, step_size):
    """Rounds a price to the nearest valid step size."""
    if not price:
        return None
...
async def execute_signal_trade(data):
    ...
    # Check for TP1 (required for scaled strategy)
    if not tp1_price:
        validation_errors.append("NO TAKE PROFIT 1 (TP1) in signal - Required for scaled strategy")
    ...
    if validation_errors:
        ...
        error_msg += (
            ...
            f"   \n"
            f"   Note: Scaled strategy requires TP1+SL minimum.\n"
            f"{'='*50}"
        )
        return error_msg
