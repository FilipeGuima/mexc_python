...
async def monitor_trade(symbol: str, start_vol: int, targets: list = None):
    import time
                ...
                if stop_res.success and stop_res.data:
                    ...
                    if sorted_stops:
                        ...
                        if isinstance(last_stop, dict):
                            ...
                            trig_side = last_stop.triggerSide.value if hasattr(last_stop.triggerSide,
                                                                               'value') else last_stop.triggerSide
...
async def resume_monitoring():
    ...
    pos_res = await MexcAPI.get_open_positions()
    ...
    if pos_res.success and pos_res.data:
        ...
        for pos in pos_res.data:
            ...
            asyncio.create_task(monitor_trade(symbol, vol, targets=[]))
            count += 1
