...
# --- CONFIGURATION ---
TELEGRAM_TOKEN = "8356806844:AAEWUVlTrH9h-3i2q7vYqH1KJZudgR9sF7w"
MEXC_TOKEN = "WEBa0f89625eb46421a336f0810447814d425d211699f40b9f79508d894d32a8a69"
...
async def partially_close_trade(symbol: str, percentage: float):
    try:
        ...
        if close_response.success:
            return {"success": True,
                    "message": f"Successfully placed order to close {percentage}% ({vol_to_close} contracts) of {symbol}."}
        else:
...
async def get_account_stats():
    try:
        ...
        while True:
            ...
            all_positions.extend(history_response.data)


            last_trade_in_page = history_response.data[-1]
        ...
        if all_positions:
            ...
            for position in all_positions:
                ...
                if close_time > thirty_days_ago:
                    ...
                    pnl_last_30_days += position.realised

                    if time_difference.days < 7:
                        pnl_last_7_days += position.realised

                    if time_difference.days < 1:
                        pnl_last_day += position.realised
...
---
<b>Realized PNL (24h):</b> <code>{pnl_last_day:.2f} USDT</code>
<b>Realized PNL (7d):</b> <code>{pnl_last_7_days:.2f} USDT</code>
<b>Realized PNL (30d):</b> <code>{pnl_last_30_days:.2f} USDT</code>
"""
...
async def get_last_position_stats(pair: str):
    try:
        open_positions_response = await API.get_open_positions(symbol=pair)
        if open_positions_response.success and open_positions_response.data:
            pos = open_positions_response.data[0]
            ...
            pnl_value = 0.0
            ticker_response = await API.get_ticker(pos.symbol)
            contract_details_response = await API.get_contract_details(pos.symbol)

            if ticker_response.success and contract_details_response.success:
                current_price = ticker_response.data.get('lastPrice', 0)
                contract_size = contract_details_response.data.get('contractSize', 1)
                entry_price = pos.openAvgPrice
                position_volume = pos.holdVol
...
<b>Close Time:</b> <code>N/A</code>
<b>Open Price:</b> <code>{pos.openAvgPrice}</code>
<b>Close Price:</b> <code>N/A</code>
<b>Unrealized PNL:</b> <code>{pnl_value:.2f} USDT</code>
"""

        history_response = await API.get_historical_positions(symbol=pair)
        if history_response.success and history_response.data:
            last_pos = history_response.data[0]
            ...
            return f"""
<b>Last Position Stats (CLOSED)</b>
...
"""
        ...
        return f"No open or historical position found for <code>{pair}</code>."
    except Exception as e:
        logger.error(f"Error in get_last_position_stats: {e}")
        return f"Error fetching last position stats: {e}"
...
<code>/stats</code> - Gets account and last position stats.
...
# --- UPDATED STATS COMMAND HANDLER ---
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(f"Fetching stats for account and last <code>{DEFAULT_PAIR}</code> position...")

    account_message, last_pos_message = await asyncio.gather(
        get_account_stats(),
        get_last_position_stats(DEFAULT_PAIR)
    )
...
# --- CONFIGURATION ---
TELEGRAM_TOKEN = "8356806844:AAEWUVlTrH9h-3i2q7vYqH1KJZudgR9sF7w"
MEXC_TOKEN = "WEBefb8b833272b0d9d3eb7b7aa21ec45172e6cfcb3340648af69f112adc7950420"
